...
# 版本：v1.7.0
# 更新日志：
#   v1.7.0 (2025-12-26): 新增国际化规范（强制），Vue 项目默认要求中英双语支持
#   v1.6.1 (2025-12-26): 修复全局 __wrapper 宽度影响分页器问题，明确禁止设置内部组件宽度
...
- **核心要求**: Composition API、TypeScript、响应式最佳实践、**国际化优先**

### 国际化规范（强制）

**⚠️ 开发任何新页面前，必须先检查项目国际化方案：**

1. **检测国际化配置**：
   - 检查 `src/locales/`、`src/i18n/`、`src/lang/` 目录
   - 查找 `messages.ts`、`zh-CN.ts`、`en-US.ts` 等文件
   - 确认是 vue-i18n 还是自定义 $t 方法

2. **遵循项目标准**：
   - **vue-i18n**：使用 `$t('common.save')` 格式
   - **自定义方案**：使用 `$t('保存')` 格式
   - 查看现有页面确认使用方式

3. **必须国际化的内容**：
   - ✅ 所有按钮文字：`<el-button>{{ $t('新增') }}</el-button>`
   - ✅ 表单标签：`:label="$t('客户名称')"`
   - ✅ 占位符：`:placeholder="$t('请输入客户名称')"`
   - ✅ 表格列标题：`:label="$t('订单编号')"`
   - ✅ 弹窗标题：`:title="$t('新增客户')"`
   - ✅ 提示消息：`ElMessage.success($t('保存成功'))`
   - ✅ 验证规则：`message: $t('请输入用户名')`
   - ✅ 下拉选项：通过 computed 返回 `{ label: $t('待处理') }`
...
- **核心要求**:
  ...
  - 避免直接选择 `.el-input`、`.el-select` 等基础组件，防止影响复合组件内部布局  - **禁止设置内部组件宽度**：\`.el-input__wrapper\`、\`.el-select__wrapper\` 等 \`__wrapper\` 组件只能设置高度、内边距、背景色，不能设置宽度（会影响分页器、日期选择器等）  - 可使用工具类：`.ml_8`、`.mr_16`、`.w_180`、`.mt_16` 等
